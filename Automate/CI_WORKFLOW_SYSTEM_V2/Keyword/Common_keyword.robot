...
CONVERT DATE FORMAT
    ...
    ELSE
        BuiltIn.Log    FuLL original date>${original_date}    WARN
        ${oper1}    BuiltIn.Evaluate    "${original_date}".find('-')
        ${oper2}    BuiltIn.Evaluate    "${original_date}".find('/')

        IF  ${oper1} > 0 and ${oper2} < 0
            BuiltIn.Log   CASE_>-    WARN
        ...
        ELSE
            ...
            # 02/12/2567
            ${new_date}    String.Split String    ${date}   /
            ${day}    BuiltIn.Convert To String    ${new_date}[0]    
            ${month}    BuiltIn.Convert To String    ${new_date}[1] 
            ${year}    BuiltIn.Convert To String    ${new_date}[2]
    ...
    BuiltIn.Log   OUTPUT>${converted_date}    WARN
...
GET DATE IN FORMATTED DATE TIME PLUS
    [Documentation]    Set date time format by fotmat thai date
    [Arguments]        ${format_date}=%d/%m/%Y    ${add_date}=00
    ${current_date}    DateTime.Get Current Date    result_format=${format_date}   
    ${current_date}    String.Split String    ${current_date}    /
    ${current_thai_year}    BuiltIn.Evaluate    ${current_date}[2] + 543
    ${current_date_plus}    BuiltIn.Evaluate    ${current_date}[0] + 1
    ${current_thai_date}    BuiltIn.Set Variable    ${current_date_plus}/${current_date}[1]/${current_thai_year}
    BuiltIn.Log    >NEW DATE>${current_thai_date}    WARN
    RETURN    ${current_thai_date}


CONVERT GET DATE IN FORMATTED DATE TIME PLUS
    [Arguments]        ${format_date}
    ${current_date}    String.Split String    ${format_date}    /
    ${current_thai_year}    BuiltIn.Evaluate    ${current_date}[2]
    ${current_date_plus}    BuiltIn.Evaluate    ${current_date}[0] + 1
    ${current_thai_date}    BuiltIn.Set Variable    ${current_date_plus}/${current_date}[1]/${current_thai_year}
    BuiltIn.Log    >CONVERT TO>${current_thai_date}    WARN
    RETURN    ${current_thai_date}
...
GET DATA JSON GET ONLY KEY
    ...
    ${indexed_keys}    Collections.Get Dictionary Keys    ${keys_group}    sort_keys=False   
    RETURN    ${indexed_keys}     
...
WAIT FOR BUTTON TO BE DISABLED
    ...
    BuiltIn.Run Keyword If    '${attribute_value}' == 'None'    BuiltIn.Return From Keyword    False
    RETURN    True
...
BUTTON SHOULD BE DISABLED
    [Arguments]    ${locator}
    ${attribute_value}    SeleniumLibrary.Get Element Attribute    ${locator}    disabled
    BuiltIn.Run Keyword If    '${attribute_value}' == 'None'    BuiltIn.fail    Button is not disabled yet
...
GET TEXT PAGE CONTAINS ELEMENT BY LOCATOR STRIP AND EQUAL STRINGS 
    ...
    [Documentation]    # if ! none ทำ 
    IF  '${data_excel}' != 'none'
        SeleniumLibrary.Wait Until Page Contains Element    ${locator}    timeout=60s
        ${data_from_locator}    SeleniumLibrary.Get Text    ${locator}
        ${result}    String.Strip String    ${data_from_locator}
        BuiltIn.Log To Console    '\nData Strip->${result}
        BuiltIn.Log    Data Strip->${result}    WARN
...
# Work set timeout
GET TEXT VERIFY DATA AND PAGE REPLACE LOCATOR OPTION AND EQUAL STRINGS
    [Arguments]     ${locator_option}    ${data_excel}    ${timeout}=15s
    IF  '${data_excel}' != 'none'
...
# Work set timeout
GET TEXT VERIFY DATA AND PAGE REPLACE LOCATOR OPTION AND EQUAL STRINGS SIGLE QUOTE
...
GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE
    ...
    # BuiltIn.Log To Console    Data Web: ${data_web} / Data Excel: ${data_excel}
...
GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL
    ...
    # BuiltIn.Log    Data Web: ${data_web}    WARN 
    IF  "${data_excel}" == "none"
        BuiltIn.Log To Console    '\nSkip
    ELSE IF  "${data_web}"=="${data_excel}"
        BuiltIn.Should Be Equal    ${data_web}    ${data_excel}
        # BuiltIn.Log    Data Web: ${data_web} / Data Excel: ${data_excel}    WARN
        # BuiltIn.Log To Console    Data Web: ${data_web} / Data Excel: ${data_excel}
    ELSE
        BuiltIn.fail     Data compare is not equal\nData Web: ${data_web} / Data Excel: ${data_excel}
    END
...
Convert To Desired Format
    [Arguments]    ${number}
    ${part1}    Evaluate    str(${number})[0:4]
    ${part2}    Evaluate    str(${number})[4:7]
    ${part3}    Evaluate    str(${number})[7:]
