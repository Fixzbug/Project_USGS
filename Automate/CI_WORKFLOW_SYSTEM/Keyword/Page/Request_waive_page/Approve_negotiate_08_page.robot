...
SET FORMAT DATA TYPE 08
    ...
    FOR  ${index}  IN RANGE    ${start_at_case}    ${countrow} + 1  
        ...
        IF  ${Request_No} != 'none' and ${State} == 'Request' or ${Request_No} != 'none' and ${State} == 'Approve' 
            ...
            Approve_negotiate_08_feature.VERIFY FEATURE - DATA TITLE - PAGE INBOX - [TYPE: 08]     # verify title 08 - inbox
            Approve_negotiate_08_feature.SET FEATURE - INPUT DATA - PAGE INBOX - [TYPE: 08]        # Input req type, reqter, req no,cust id and cont no
            # Approve_negotiate_08_feature.VERIFY FEATURE - DATA TITLE TABLE - PAGE INBOX - [TYPE: 08]     # verify title table - inbox
            Approve_negotiate_08_feature.VERIFY FEATURE - DATA VALUE TABLE - PAGE INBOX - [TYPE: 08]     # verify value table - inbox
            ...
            # Approve Negotiate
            # Approve_negotiate_08_feature.VERIFY FEATURE - DATA TITLE - PAGE APPROVE NEGOTIATE - [TYPE: 08]    # verify title - approve negotiate blog 1
            Approve_negotiate_08_feature.VERIFY FEATURE - DATA VALUE - PAGE APPROVE NEGOTIATE - [TYPE: 08]    # verify value - approve negotiate blog 1

            # # 1. Negotiate discount detail
            # Approve_negotiate_08_feature.VERIFY FEATURE - DATA TITLE - PAGE APPROVE NEGOTIATE [NEGOTIATE DETAIL] - [1. ENTRY NEGOTIATE DISCOUNT DETAIL] - [TYPE: 08]    # verify title 08 - 1
            Approve_negotiate_08_feature.VERIFY FEATURE - DATA VALUE - PAGE APPROVE NEGOTIATE [NEGOTIATE DETAIL] - [1. ENTRY NEGOTIATE DISCOUNT DETAIL] - [TYPE: 08]    # verify value 08 - 1    [capture]

            # # 3. Request detail
            # Approve_negotiate_08_feature.VERIFY FEATURE - DATA TITLE - PAGE APPROVE NEGOTIATE [NEGOTIATE DETAIL] - [3. REQUEST DETAIL] - [TYPE: 08]    # verify title 08 - 3
            Approve_negotiate_08_feature.VERIFY FEATURE - DATA VALUE - PAGE APPROVE NEGOTIATE [NEGOTIATE DETAIL] - [3. REQUEST DETAIL] - [TYPE: 08]    # verify value 08 - 3
...
# add new
PAGE INBOX - TYPE 08 - OPEN INRANET
    Common_keyword.OPEN INTRANET
    Common_keyword.CLICK SSO ICON

# add new
PAGE INBOX - TYPE 08 - INPUT USENAME AND PASSWORD BY USER LEVEL
    [Arguments]    ${user_level}

    IF  '${user_level}' == '0'         # user approve user0 
        PAGE INBOX - TYPE 08 - SETTING LOGIN    WORKFLOW    ${group_user}[${keys_group_user}[3]]    ${group_user}[${keys_group_user}[4]]    ${user_level}    # 0 row,col -> 3,4 -> P2503064
    ELSE IF  '${user_level}' == '1'    # user approve user1 
        PAGE INBOX - TYPE 08 - SETTING LOGIN    WORKFLOW    ${group_user}[${keys_group_user}[5]]    ${group_user}[${keys_group_user}[6]]    ${user_level}    # 1 row,col -> 5,6 -> P2510033
    ELSE IF  '${user_level}' == '2'    # user approve user2
        PAGE INBOX - TYPE 08 - SETTING LOGIN    WORKFLOW    ${group_user}[${keys_group_user}[7]]    ${group_user}[${keys_group_user}[8]]    ${user_level}     # 2 row,col -> 7,8  -> P2503067
    ELSE IF  '${user_level}' == '3'    # user approve user3
        PAGE INBOX - TYPE 08 - SETTING LOGIN    WORKFLOW    ${group_user}[${keys_group_user}[9]]    ${group_user}[${keys_group_user}[10]]    ${user_level}     # 3 row,col -> 9,10 -> P2106010 
    ELSE IF  '${user_level}' == '4'    # user approve user4
        PAGE INBOX - TYPE 08 - SETTING LOGIN    WORKFLOW    ${group_user}[${keys_group_user}[11]]    ${group_user}[${keys_group_user}[12]]    ${user_level}     # 4 row,col -> 11,12 -> KIMURA
    END

# add new
PAGE INBOX - TYPE 08 - SETTING LOGIN
...
# wait update page 
PAGE INBOX - TYPE 08 - [WAIT] - [VERIFY] PAGE
    Common_keyword.WAITING FOR DATA UPDATE    1s    

PAGE INBOX - TYPE 08 - [CLICK] - [MENU]
    [Arguments]    ${menu} 
...
PAGE INBOX - [INPUT] - REQUEST TYPE
    Common_keyword.WAITING ELEMENT WHEN ELEMENT IS VISIBLE BY LOCATOR    ${dropdown_08_inbox_req_type_locator}
    SeleniumLibrary.Select From List By Label   ${dropdown_08_inbox_req_type_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[1]]]


PAGE INBOX - [INPUT] - REQUESTER
    Common_keyword.INPUT TEXT WHEN ELEMENT IS VISIBLE    ${txt_08_inbox_requester_locator}    ${group_data}[${group_data_label}[${keys_group_data_label}[6]]]


PAGE INBOX - [INPUT] - REQUEST NO.
    Common_keyword.INPUT TEXT WHEN ELEMENT IS VISIBLE    ${txt_08_inbox_request_no_locator}    ${group_data}[${group_data_label}[${keys_group_data_label}[5]]]


PAGE INBOX - [INPUT] - CUSTOMER ID
    Common_keyword.INPUT TEXT WHEN ELEMENT IS VISIBLE    ${txt_08_inbox_cust_id_locator}    ${group_data}[${group_data_label}[${keys_group_data_label}[2]]]


PAGE INBOX - [INPUT] - CONTRACT NO
    Common_keyword.INPUT TEXT WHEN ELEMENT IS VISIBLE    ${txt_08_inbox_contract_locator}    ${group_data}[${group_data_label}[${keys_group_data_label}[4]]]


PAGE INBOX - [CLICK] - [BUTTON] - SEARCH
    Common_keyword.CLICK ELEMENT WHEN ELEMENT IS VISIBLE    ${btn_08_inbox_search_locator}



# Verify Title Table - Page Inbox
PAGE INBOX - [VERIFY] - [TITLE][TABLE] - NO
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    No


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - REQ NO.
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[94]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - SENDER
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[95]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - SEND DATE
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[96]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - REQUEST TYPE
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_req_type_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[1]]

PAGE INBOX - [VERIFY] - [TITLE][TABLE] - REQUEST TYPE 2
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_req_type2_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[1]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - CUSTOMER ID
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[2]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - CONTRACT NO.
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[97]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - NAME
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[98]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - REQUEST BY
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[99]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - TEAM
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[11]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - REASON
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[90]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - REQUEST AMT
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[100]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - APPROVE USER
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[101]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - STATUS
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    ${group_data_label}[${keys_group_data_label}[102]]


PAGE INBOX - [VERIFY] - [TITLE][TABLE] - ACTION
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_table_08_inbox_format_locator}    Action



# Verify Value Table - Page Inbox
PAGE INBOX - [VERIFY] - [VALUE][TABLE] - NO
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     1


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - REQ NO.
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[5]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - SENDER
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[95]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - SEND DATE
    ${current_date_from_local}    common_keyword.GET DATE IN FORMATTED DATE TIME
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${current_date_from_local}
    # ${date_send_date}    Common_keyword.CONVERT DATE FORMAT    ${group_data}[${group_data_label}[${keys_group_data_label}[96]]]
    # Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${date_send_date}


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - REQUEST TYPE
    ${result0}    GET AND SPLIT TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE - [TYPE: 08]    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[1]]]    0

PAGE INBOX - [VERIFY] - [VALUE][TABLE] - REQUEST TYPE 2
    ${result1}    GET AND SPLIT TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE - [TYPE: 08]    ${txt_value_table_req_type_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[1]]]    1


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - CUSTOMER ID
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[2]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - CONTRACT NO.
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[97]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - NAME
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[98]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - REQUEST BY
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[99]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - TEAM
    BuiltIn.Run Keyword And Ignore Error    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[11]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - REASON
    ${result0}    GET AND SPLIT TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE - [TYPE: 08]    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[90]]]    0   


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - REQUEST AMT
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[100]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - APPROVE USER
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL    ${txt_value__appr_user_08_inbox_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[101]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - STATUS
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_08_inbox_format_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[102]]]


PAGE INBOX - [VERIFY] - [VALUE][TABLE] - ACTION
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_value_table_action_08_inbox_format_locator}    Action



# Click link action in table
PAGE INBOX - [CLICK] - [TEXT] - ACTION
    Common_keyword.CLICK ELEMENT WHEN ELEMENT IS VISIBLE    ${txt_herf_table_action_08_inbox_locator}
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - REQUEST NO.
    Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL    ${txt_value_08_nsd_request_no_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[5]]]    value
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - REQUEST DATE
    ...
    # Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA DATE FROM EXCEL    ${txt_value_08_nsd_request_date_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[7]]]    value
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - REQUEST STATUS
    Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL    ${txt_value_08_nsd_req_status_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[102]]]    value
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - STATUS DATE
    ${current_date_from_local}    common_keyword.GET DATE IN FORMATTED DATE TIME
    Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL    ${txt_value_08_nsd_status_date_locator}     ${current_date_from_local}	value
    # ${status_date}    Common_keyword.CONVERT DATE FORMAT    ${group_data}[${group_data_label}[${keys_group_data_label}[7]]]
    # Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL    ${txt_value_08_nsd_status_date_locator}      ${status_date}    value
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - NUMBER OF CHILD
    Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL    ${input_08_nsd_num_of_child_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[12]]]    value


PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - TEAM
    Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL    ${txt_value_08_nsd_team2_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[11]]]    value
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - OD STATUS
    Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL    ${txt_value_08_nsd_od_status2_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[14]]]    value


PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - LAST SALARY
    Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL AND REPLACE COMMA    ${input_08_nsd_last_salary_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[15]]]    value
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - CUSTOMER STATUS
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL    ${txt_value_08_nsd_cust_status_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[17]]]
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - LAST REQUEST NO
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL    ${txt_value_08_nsd_last_req_no_locator}     ${group_data}[${group_data_label}[${keys_group_data_label}[25]]]
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - LAST CONTACT THIRD PARTY [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_last_cont_3_party_locator}    ${group_data_label}[${keys_group_data_label}[83]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - LAST CONTACT CUSTOMER [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_last_cont_cust_locator}    ${group_data_label}[${keys_group_data_label}[84]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - HAVE JOB [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_have_job_locator}    ${group_data_label}[${keys_group_data_label}[85]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - WHO REQUEST [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_who_req_locator}    ${group_data_label}[${keys_group_data_label}[86]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - DISCOUNT CAMPAIGN [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_disc_campaign_locator}    ${group_data_label}[${keys_group_data_label}[87]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - WHO PAID [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_who_paid_locator}    ${group_data_label}[${keys_group_data_label}[88]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - SOURCE OF FUND [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_source_fund_locator}    ${group_data_label}[${keys_group_data_label}[89]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - REASON [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_reason_locator}    ${group_data_label}[${keys_group_data_label}[90]]
    

PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - REASON NOTE [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_reason_note_locator}    ${group_data_label}[${keys_group_data_label}[91]]
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - SEND TO [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    ...
    IF    '${result}' == 'True'
        Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_send_to_locator}    ${group_data_label}[${keys_group_data_label}[92]]
    END


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - REJECT REASON [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_rej_reason_locator}    ${group_data_label}[${keys_group_data_label}[103]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - REJECT NOTE [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_rej_note_locator}    ${group_data_label}[${keys_group_data_label}[104]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [TITLE] - NOTE [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL BY REPLACE VAULE    ${txt_title_approve_negotiate_note_locator}    ${group_data_label}[${keys_group_data_label}[93]]
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - LAST CONTACT THIRD PARTY [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    ...
    #     BuiltIn.Log    !!!! USING DATE IN SYSTEM !!!!    WARN
    #     SeleniumLibrary.Execute JavaScript    document.getElementById('${input_08_nd_js_last_cont_3_party_locator}').value='${date_start_cal_date}';
    # ELSE
    #     BuiltIn.Log    !!!! USING DATE IN EXCEL !!!!    WARN
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - LAST CONTACT CUSTOMER [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    ...
    #     BuiltIn.Log    !!!! USING DATE IN SYSTEM !!!!    WARN
    #     SeleniumLibrary.Execute JavaScript    document.getElementById('${input_08_nd_js_last_cont_cust_locator}').value='${date_start_cal_date}';
    # ELSE
    #     BuiltIn.Log    !!!! USING DATE IN EXCEL !!!!    WARN
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - WHO REQUEST [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    SeleniumLibrary.Wait Until Element Is Visible    ${txt_08_nd_who_req_locator}   15
    ${data_web}    SeleniumLibrary.Get Element Attribute    ${txt_08_nd_who_req_locator}    value
    ...
    ${data_excel}    BuiltIn.Set Variable If      "${group_data}[${group_data_label}[${keys_group_data_label}[86]]]" == "none"      Data is empty    ${modified_data_web}
...
PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - DISCOUNT CAMPAIGN [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL WITH STRIP STRING       ${select_08_nd_discount_camp_locator}    ${group_data}[${group_data_label}[${keys_group_data_label}[87]]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - WHO PAID [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL WITH STRIP STRING       ${select_08_nd_who_paid_locator}    ${group_data}[${group_data_label}[${keys_group_data_label}[88]]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - SOURCE OF FUND [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL WITH STRIP STRING       ${select_08_nd_source_fund_locator}    ${group_data}[${group_data_label}[${keys_group_data_label}[89]]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - REASON [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT AND VERIFY DATA FROM EXCEL WITH STRIP STRING       ${select_08_nd_reason_locator}    ${group_data}[${group_data_label}[${keys_group_data_label}[90]]]


PAGE APPROVE NEGOTIATE - [VERIFY] - [VALUE] - REASON NOTE [NEGOTIATE DETAIL] - [3. REQUEST DETAIL]
    Common_keyword.GET TEXT BY ELEMENT ATTRIBUTE AND VERIFY DATA FROM EXCEL       ${input_08_nd_reason_note_locator}    ${group_data}[${group_data_label}[${keys_group_data_label}[91]]]    value


# verify   
PAGE APPROVE NEGOTIATE - TYPE 08 - [VERIFY] - [LABEL] SEND TO
    [Arguments]    ${user_level}
    ...
    IF  '${status}' == 'True'    # ถ้ามี ปุ่ม send to
        ...
        Approve_negotiate_08_feature.SET FEATURE - TYPE - 08 - APPROVE POPUP AND SAVE DATA - APPROVE REQUEST PENALTY - PAGE INBOX
