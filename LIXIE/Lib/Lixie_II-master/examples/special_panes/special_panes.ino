...
void double_panes()
{ // double_panes() uses special panes explicitly through lix.special_pane(uint8_t index, bool enabled, CRGB col1, CRGB col2);
  ...
  { // enable special panes in green, one by one
    lix.special_pane(i, true, CRGB(0, 255, 0));
    delay(1000);
  }

  for (uint8_t i = 0; i < NUM_DIGITS; i++)
  { // enable dual-color special panes, one by one
    lix.special_pane(i, true, CRGB(0, 255, 255), CRGB(255, 0, 0));
    delay(1000);
  }

  for (uint8_t i = 0; i < NUM_DIGITS; i++)
  { // disable special panes, one by one
...
void fake_load()
{ // fake_load() uses special panes implicitly through lix.write(String input); using spaces and periods, and respects the ON and OFF color groups
