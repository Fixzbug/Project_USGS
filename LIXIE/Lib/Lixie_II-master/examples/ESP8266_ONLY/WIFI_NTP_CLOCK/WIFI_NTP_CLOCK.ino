...
void show_time()
{
  ...
  { // Dim overnight from 9PM to 6AM (21:00 to 06:00)
    if (night_dimming_state == HIGH)
    { // But only if dimming is enabled
      lix.brightness(0.4);
    }
    else
    { // If not enabled, full brightness
      lix.brightness(1.0);
    }
  }
  else
  { // Or if not in the overnight time window, full brightness
    lix.brightness(1.0);
  }
...
void parse_buttons()
{
  if (hour_button_edge == FALL)
  { // PRESS STARTED
    hour_button_start = t_now;
  }
  else if (hour_button_edge == RISE)
  { // PRESS ENDED
    uint16_t hour_button_duration = t_now - hour_button_start;
    if (hour_button_duration < hour_button_wait)
    { // RELEASED QUICKLY
      Serial.println("UP");
  ...
  if (hue_button_edge == FALL)
  { // PRESS STARTED
    Serial.println("HUE");
    hue_button_start = t_now;
  }
  else if (hue_button_edge == RISE)
  { // PRESS ENDED
